import { observable, computed, action } from 'mobx';

class RootStore {
	constructor(){
		this.vehicleMakeModuleStore = new VehicleMakeModuleStore(this)
		this.vehicleModelModuleStore = new VehicleModelModuleStore(this)
	}
}

class VehicleMakeModuleStore {
	constructor(rootStore){
		this.rootStore = rootStore
		this.vehicleMakeListViewStore = new VehicleMakeListViewStore(this)
		this.vehicleMakeAddStore = new VehicleMakeAddStore(this)
		this.vehicleMakeEditViewStore = new VehicleMakeEditViewStore(this)
	}

	@observable makes = [
		{ id: "0", VehicleMake: "Ford Taurus", logo: "https://seeklogo.net/wp-content/uploads/2014/07/ford-logo.png" },
		{ id: "1", VehicleMake: "Toyota", logo: "https://seeklogo.net/wp-content/uploads/2014/06/toyota-logo-vector-download.jpg" },
		{ id: "2", VehicleMake: "Volkswagen Golf", logo: "data:image/png;base64,"},
		{ id: "3", VehicleMake: "BMW", logo:"https://images-na.ssl-images-amazon.com/images/I/61b2FkbZsEL._AC_SY355_.jpg" },
		{ id: "4", VehicleMake: "Honda", logo: "https://seeklogo.net/wp-content/uploads/2011/05/honda-silver-logo-vector.png"},
		{ id: "5", VehicleMake: "Alfa", logo: "data:image/png;base64,"}, 
		{ id: "6", VehicleMake: "Hyundai Kona", logo: "https://di-uploads-pod4.dealerinspire.com/rivercityhyundai/uploads/2017/05/HyundaiLogoStacked_4cblk-1024x659.gif" }
	]

	//delete a car by id 
	@action removeCar = (id) => {
		//had to replace the splice method so that the cars array isn't mutated after deleting 
		this.makes[id] = null
  	}

}

class VehicleMakeListViewStore {
	constructor(vehicleMakeModuleStore){
		this.vehicleMakeModuleStore = vehicleMakeModuleStore
	}

	@observable filter = ""
	@observable isSorted = false;

	@observable currentPage = 1
  	@observable carsPerPage = 6
  	@observable indexOfLastCar = (this.currentPage * this.carsPerPage)
  	@observable indexOfFirstCar = (this.indexOfLastCar - this.carsPerPage)

  	//list only car makes
	@computed get listMakes(){
		const matchesFilter = new RegExp(this.filter, "i")
		return this.vehicleMakeModuleStore.makes.filter(car => car !== null).filter(car => !this.filter || matchesFilter.test(car.VehicleMake))
	}

	//list alphabetically sorted makes
	@computed get sortedMakes() {
    	return this.listMakes.filter(car => car !== null).slice().sort((a, b) => (a.VehicleMake > b.VehicleMake) ? 1 : -1);
  	}

  	//show makes on current page
  	@computed get currentCars (){
  		return this.listMakes.slice(this.indexOfFirstCar, this.indexOfLastCar)
  	}

  	//get sorted makes on current page
  	@computed get currentSortedCars (){
  		return this.sortedMakes.slice(this.indexOfFirstCar, this.indexOfLastCar)
  	}

  	//set the page to be whatever number is clicked on in Pagination component
  	@action setPage = (pageNumber) => {
  		this.currentPage = pageNumber
  		this.indexOfLastCar = (this.currentPage * this.carsPerPage)
  		this.indexOfFirstCar = (this.indexOfLastCar - this.carsPerPage)
  	}
}

class VehicleMakeAddStore {
	constructor(vehicleMakeModuleStore){
		this.vehicleMakeModuleStore = vehicleMakeModuleStore
	}

	// save id of last car so that the user doesn't have to manually insert it
	@observable lastId = this.vehicleMakeModuleStore.makes.slice(-1)[0].id

	@observable newMake = ''
	@observable newLogo = ''
	
  	//add a new car 
	@action addCar = (newMake, newLogo) => {
		console.log(newMake, newLogo)
		this.vehicleMakeModuleStore.makes.push({
			id: ++this.lastId, 
			VehicleMake: newMake, 
			logo: newLogo
		})
	}
}

class VehicleMakeEditViewStore {
	constructor(vehicleMakeModuleStore){
		this.vehicleMakeModuleStore = vehicleMakeModuleStore
	}

	@action editMake = (id, make, logo) => {
		this.vehicleMakeModuleStore.makes[id].VehicleMake = make
		this.vehicleMakeModuleStore.makes[id].logo = logo
	}
}

class VehicleModelModuleStore {
	constructor(rootStore){
		this.rootStore = rootStore
		this.vehicleModelListViewStore = new VehicleModelListViewStore(this)
		this.vehicleModelAddStore = new VehicleModelAddStore(this)
		this.vehicleModelEditViewStore = new VehicleModelEditViewStore(this)
	}

	@observable models = [
		{ id: "0", VehicleModel: "SHO", image:"https://cars.usnews.com/static/images/Auto/izmo/i10477998/2017_ford_taurus_angularfront.jpg" },
		{ id: "1", VehicleModel: "SE (6MT)", image:"https://s.aolcdn.com/commerce/autodata/images/USD00TOC041G021001.jpg" },
		{ id: "2", VehicleModel: "MSRP", image: "https://tdrvehicles2.azureedge.net/photos/chrome/Expanded/White/2019VWC02/2019VWC02000101.jpg" },
		{ id: "3", VehicleModel: "M135i xDrive", image: "https://ymimg1.b8cdn.com/resized/car_model/4597/pictures/4021192/mobile_listing_main_01.jpg" },
		{ id: "4", VehicleModel: "Civic", image: "https://cache4.pakwheels.com/system/car_generation_pictures/4962/original/Honda_Civic_Facelift_2019.jpg" },
		{ id: "5", VehicleModel: "Romeo Stelvio", image: "https://images.dealer.com/ddc/vehicles/2018/Alfa%20Romeo/Stelvio/SUV/trim_Base_1a03c3/color/Rosso%20Alfa-PRR-135%2C14%2C50-640-en_US.jpg"}, 
		{ id: "6", VehicleModel: "SEL", image:"https://www.whitescanyonford.com/assets/stock/colormatched_01/white/640/cc_2016foc070001_01_640/cc_2016foc070001_01_640_yz.jpg" }, 
		{ id: "7", VehicleModel: "SEL", image: "https://m.media-amazon.com/images/I/71n9O408ZTL._UY560_.jpg" },
		{ id: "8", VehicleModel: "i8", image: "https://auto.ndtvimg.com/car-images/large/bmw/i8/bmw-i8.webp?v=7" },
		{ id: "9", VehicleModel: "Yaris", image: "https://dealerimages.dealereprocess.com/image/upload/c_limit,f_auto,fl_lossy,w_600/v1/svp/dep/17toyotayarisiafwdsa1a/toyota_17yarisiafwdsa1a_angularfront_abyss" }
	]

	//delete a car by id 
	@action removeCar = (id) => {
		this.models[id] = null
  	}
}

class VehicleModelListViewStore {
	constructor(vehicleModelModuleStore){
		this.vehicleModelModuleStore = vehicleModelModuleStore
	}

	@observable filter = ""
	@observable isSorted = false

	@observable currentPage = 1
  	@observable carsPerPage = 6
  	@observable indexOfLastCar = (this.currentPage * this.carsPerPage)
  	@observable indexOfFirstCar = (this.indexOfLastCar - this.carsPerPage)

	//list only car models
	@computed get listModels(){
		const matchesFilter = new RegExp(this.filter, "i")
		return this.vehicleModelModuleStore.models.filter(car => car !== null).filter(car => !this.filter || matchesFilter.test(car.VehicleModel))
	}

	//list alphabetically sorted models
	@computed get sortedModels() {
    	return this.listModels.filter(car => car !== null).slice().sort((a, b) => (a.VehicleModel > b.VehicleModel) ? 1 : -1);
  	}

  	//show models on current page
  	@computed get currentCars (){
  		return this.listModels.slice(this.indexOfFirstCar, this.indexOfLastCar)
  	}

  	//get sorted models on current page
  	@computed get currentSortedCars (){
  		return this.sortedModels.slice(this.indexOfFirstCar, this.indexOfLastCar)
  	}

  	//set the page to be whatever number is clicked on in Pagination component
  	@action setPage = (pageNumber) => {
  		this.currentPage = pageNumber
  		this.indexOfLastCar = (this.currentPage * this.carsPerPage)
  		this.indexOfFirstCar = (this.indexOfLastCar - this.carsPerPage)
  	}
}

class VehicleModelAddStore {
	constructor(vehicleModelModuleStore){
		this.vehicleModelModuleStore = vehicleModelModuleStore	
	}

	// save id of last car so that the user doesn't have to manually insert it
	@observable lastId = this.vehicleModelModuleStore.models.slice(-1)[0].id

	@observable newModel = ''
	@observable newImage = ''
	
  	//add a new car 
	@action addCar = (newModel, newImage) => {
		this.vehicleModelModuleStore.models.push({
			id: ++this.lastId, 
			VehicleModel: newModel, 
			image: newImage
		})
	}
}

class VehicleModelEditViewStore {
	constructor(vehicleModelModuleStore){
		this.vehicleModelModuleStore = vehicleModelModuleStore
	}

	@action editModel = (id, model, image) => {
		this.vehicleModelModuleStore.models[id].VehicleModel = model
		this.vehicleModelModuleStore.models[id].image = image
	}

}

const rootStore = new RootStore()
export default rootStore